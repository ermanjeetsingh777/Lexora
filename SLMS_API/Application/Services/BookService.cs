...
public class BookService : IBookService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var items = books
            .Select(b =>
            {
                ...
                var overdue = activeLoans.Count(l => ResolveLoanStatus(l, now) == BookLoanStatus.Overdue);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new BookStatsResponse
        {
            ...
            OnLoanCount = activeLoans.Count,
            OverdueCount = activeLoans.Count(l => ResolveLoanStatus(l, now) == BookLoanStatus.Overdue),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var status = ResolveLoanStatus(l, now);
            var overdueDays = status == BookLoanStatus.Overdue ? ComputeOverdueDays(l.DueAtUtc, now) : l.OverdueDays ?? 0;
            var finePerDay = fineRates.TryGetValue(l.LibraryId, out var rate) ? rate : 10m;
            var fineAmount = l.FineAmount ?? (status == BookLoanStatus.Overdue ? overdueDays * finePerDay : 0m);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var status = ResolveLoanStatus(loan, now);
    ...
    private static BookLoanStatus ResolveLoanStatus(BookLoan loan, DateTime now)
    {
        if (loan.Status == BookLoanStatus.Returned || loan.ReturnedAtUtc.HasValue)
    ...
    private static BookDetailResponse MapDetail(Book book)
    {
        ...
        return new BookDetailResponse
        {
            ...
            Activities = book.Loans
                ...
                {
                    var status = ResolveLoanStatus(l, now);
                    var isActiveLoan = status is BookLoanStatus.Active or BookLoanStatus.Overdue;
                    var daysOverdue = status == BookLoanStatus.Overdue ? ComputeOverdueDays(l.DueAtUtc, now) : 0;
                    var items = new List<BookActivityResponse>
                    {
                        new()
                        {
                            ...
                            DueAtUtc = l.DueAtUtc,
                            LoanDays = l.LoanDays > 0 ? l.LoanDays : 14,
                            IsOverdue = status == BookLoanStatus.Overdue,
                            DaysOverdue = daysOverdue,
                            EstimatedFine = l.FineAmount ?? (daysOverdue > 0 ? daysOverdue * 10m : 0m),
                        },
                    };
                    ...
                    else if (status == BookLoanStatus.Overdue)
                    {
                        items.Add(new BookActivityResponse
                        {
                            Id = Guid.NewGuid(),
