...
export class BookService {
  ...
  getMemberDigitalBooks(memberId: string, params?: { search?: string; category?: string }): Observable<APIResponseModel<BookListItem[]>> {
    const query = new URLSearchParams();
    if (params?.search) query.set('search', params.search);
    if (params?.category) query.set('category', params.category);
    const suffix = query.toString() ? `?${query}` : '';
    return this.http.get<BookListItem[]>(`members/${memberId}/digital-books${suffix}`);
  }

  downloadMemberDigitalPdf(memberId: string, bookId: string): Observable<Blob> {
    return this.http.download(`members/${memberId}/digital-books/${bookId}/pdf`);
  }
}
