...
import { LucideDownload, LucideMail, LucideMessageCircle } from '@lucide/angular';
...
@Component({
  ...
  imports: [GlassCardComponent, AppDatePipe, StatusBadgeComponent, LucideDownload, LucideMail, LucideMessageCircle],
...
export class MemberPaymentsComponent {
  ...
  shareEmail(plan?: MemberPlanResponse): void {
    ...
    if (!email) {
      this.toast.error('Member email address is not available.');
      return;
    }
    const allPlans = this.plans() ?? [];
    if (allPlans.length === 0) {
